...
def ddl_downloader(queue):
    ...
    while True:
        if mylar.DDL_LOCK is True:
            ...
            if item == 'startup':
                try:
                    pending_ddl = myDB.select(
                        "SELECT * FROM ddl_info WHERE status in ('Queued','Downloading')"
                    )
                except Exception as e:
                    logger.warn('[DDL-QUEUE] Unable to repopulate DDL queue: %s' % e)
                else:
                    logger.info('[DDL-QUEUE] Repopulating DDL queue with %s previously incomplete item(s)' % len(pending_ddl or []))
                    for dl in pending_ddl or []:
                        dl_data = dict(dl)
                        # ddl_info declares the column as 'ID' - dict(sqlite3.Row) preserves
                        # that case, and dict.get is case-sensitive. Check both so the record
                        # id can never come back as None (which kills this thread downstream).
                        record_id = dl_data.get('ID') or dl_data.get('id')
                        if record_id is None:
                            logger.warn('[DDL-QUEUE] Skipping malformed ddl_info record with no id: %s' % dl_data)
                            continue
                        filename = dl_data.get('tmp_filename') or dl_data.get('filename')
                        resume = None
                        if all([mylar.CONFIG.DDL_AUTORESUME is True,
                                dl_data.get('status') == 'Downloading',
                                dl_data.get('link_type') in (None, 'GC-Main', 'GC-Mirror')]):
                            try:
                                resume = os.stat(os.path.join(mylar.CONFIG.DDL_LOCATION, filename)).st_size
                            except Exception:
                                logger.fdebug('[DDL-QUEUE] No partial file for %s - will restart download.' % filename)
                                resume = None
                        queue.put({'link':            dl_data.get('link'),
                                   'mainlink':        dl_data.get('mainlink'),
                                   'series':          dl_data.get('series'),
                                   'year':            dl_data.get('year'),
                                   'size':            dl_data.get('size'),
                                   'comicid':         dl_data.get('comicid'),
                                   'issueid':         dl_data.get('issueid'),
                                   'oneoff':          dl_data.get('oneoff'),
                                   'id':              record_id,
                                   'link_type':       dl_data.get('link_type'),
                                   'filename':        filename,
                                   'comicinfo':       None,
                                   'packinfo':        None,
                                   'site':            dl_data.get('site') or 'DDL(GetComics)',
                                   'remote_filesize': dl_data.get('remote_filesize') or 0,
                                   'resume':          resume})
                continue

            # A single malformed/poisoned queue item must never kill this worker.
            # If it escapes here, the whole DDL queue stops draining until restart.
            try:
                ...
                logger.error('[DDL-QUEUE] Unhandled error processing %s [id:%s] - marking Failed and continuing: %s' % (item.get('series'), item.get('id'), e), exc_info=True)
                try:
                ...
                # never leave the global lock held by a failed item
